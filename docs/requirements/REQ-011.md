...
- **MPD Client:** `src/snapctrl/api/mpd/client.py` — Full async MPD protocol client
- **MPD Monitor:** `src/snapctrl/core/mpd_monitor.py` — Track change detection with art cache
- **Album Art Providers:** `src/snapctrl/api/album_art/` — Provider chain (iTunes, MusicBrainz)
