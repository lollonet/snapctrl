...
- **API:** `Group.SetClients` takes `{id: group_id, clients: [client_id, ...]}` — must send the full new client list for the target group (add moved client) and the source group (remove moved client)
